...
A \pkg{proto} object is an environment and the defined methods in a 
\pkg{proto} object have, by default,
...
\pkg{proto} object in which they are defined.  This is referred to as object
...
\citep{Kates2004}
which discusses the fragile base class problem which 
relates to this question. Note that if a \pkg{proto} object is created
via the \code{proto} function with an argument of \code{funEnvir = FALSE}
then the environment of the function/method will not be reset and so the
above does not apply.  This can be used for instances when the default
processing is not desirable.}
\item{Why does \code{obj\$meth} not return the method, \code{meth}?  

\code{obj\$meth(x, y)} generates a call like \code{meth(obj, x, y)} so 
\code{obj\$meth} needs to
return \code{meth} with its first argument, \code{obj}, already inserted.  Since
calling a method is the most common operation that operation was made
the simplest. To get the method itself use \code{with(obj, meth)} or
\code{obj\$with(meth)}.}
\item{How does one debug a method?

\pkg{proto} will not dynamically redefine methods.  This has the advantage
...
No. proto is just a thin layer on top of \proglang{R} 
environments and \proglang{R} environments
provide single inheritance only.  Note that \citep{Kates2004}
