...
lazy evaluation; however, the \code{proto} function itself, like the
\proglang{R} \code{list} function, does evaluate its arguments.  To get the
effect of lazy evaluation in this case replace any properties with methods.
For example, contrast this:
\begin{verbatim}
   x <- 0
   p <- proto(f = x) # f is a property of p
   x <- 1
   p$f # 0
\end{verbatim}
with this:
\begin{verbatim}
   x <- 0
   p <- proto(f = function(.) x) # f is a method of p
   x <- 1
   p$f() # 1
\end{verbatim}
}
