...
#'
#' \code{"proto"} allows one to write object-oriented programs using the
#' prototype model.  It is a simple system that uses objects rather than
#' classes yet is powerful enough to readily encompass classes too.  The
#' following are sources of information on \code{"proto"}: \tabular{ll}{ Home
#' page\tab \code{u <- "http://r-proto.googlecode.com"; browseURL(u)}\cr
#' Overview\tab \code{README <- system.file("README", package = "proto");
#' file.show(README)}\cr Invoking a demo file\tab \code{demo("proto")}\cr
#' Referance Card\tab \code{vignette("protoref")}\cr Tutorial \tab
#' \code{vignette("proto")}\cr Prototype OO concepts document\tab \code{u <-
#' "http://r-proto.googlecode.com/files/prototype_approaches.pdf";
#' browseURL(u)}\cr News\tab \code{RShowDoc("NEWS", package = "proto")}\cr Wish
#' List\tab \code{RShowDoc("WISHLIST", package = "proto")}\cr Thanks\tab
#' \code{RShowDoc("THANKS", package = "proto")}\cr License\tab
#' \code{RShowDoc("COPYING", package = "proto")}\cr Citation\tab
#' \code{citation(package = "proto")}\cr This File\tab \code{package?proto}\cr
#' Help file\tab \code{?proto}\cr graph.proto Help File\tab
#' \code{?graph.proto}\cr }
#'
#' @name proto-package
#' @docType package
#' @note See \code{?graph.proto} for the solution to a common Linux
#' installation problem with \code{Rgraphviz} that \code{proto} depends on.
